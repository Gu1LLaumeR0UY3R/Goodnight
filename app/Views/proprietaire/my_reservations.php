...
<head>
    ...
    <style>
        ...
        .dark-mode .tab-btn.active {
            background: linear-gradient(135deg, var(--night-stellar), var(--night-nebula));
            border-color: var(--night-stellar);
        }
...
<body class="home-sunset">
    ...
    <main>
        ...
        <div class="tabs-container">
            <button class="tab-btn active" onclick="switchTab('personnelles')">
                👤 Personnelles
            </button>
            <button class="tab-btn" onclick="switchTab('professionnelles')">
                🏢 Professionnelles
            </button>
        </div>

        <!-- Réservations Personnelles -->
        <div id="tab-personnelles" class="tab-content active">
            <h3 style="text-align: center; color: var(--text-secondary); margin-bottom: 1.5rem; font-size: 1.25rem;">
                Réservations que j'ai effectuées
            </h3>
            <?php if (!empty($reservationsPersonnelles)): ?>
                <table>
                    <thead>
                        <tr>
                            <th>Bien</th>
                    ...
                    <tbody>
                        <?php foreach ($reservationsPersonnelles as $reservation): ?>
                        <tr>
                            <td data-label="Bien">
                            ...
                            <td data-label="Action">
                                <form action="/reservation/cancel/<?= $reservation['id_reservation'] ?>" 
                                    ...
                                    <button type="submit" class="btn-cancel">�️ Annuler</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
            ...
            <?php else: ?>
                <p>Vous n'avez effectué aucune réservation personnelle.</p>
            <?php endif; ?>
        </div>

        <!-- Réservations Professionnelles -->
        <div id="tab-professionnelles" class="tab-content">
            <h3 style="text-align: center; color: var(--text-secondary); margin-bottom: 1.5rem; font-size: 1.25rem;">
                Réservations sur mes biens
            </h3>
            <?php if (!empty($reservationsProfessionnelles)): ?>
                <table>
                    <thead>
                        <tr>
                            <th>Bien</th>
                    ...
                    <tbody>
                        <?php foreach ($reservationsProfessionnelles as $reservation): ?>
                            <tr>
                                <td data-label="Bien">
                                    <?php if (!empty($reservation['premiere_photo'])): ?>
                                        <img src="<?= htmlspecialchars($reservation['premiere_photo']) ?>" 
                                ...
                                <td data-label="Locataire">
                                    <?= htmlspecialchars($reservation["nom_locataire"] . " " . $reservation["prenom_locataire"]) ?>
                                    <?php if (!empty($reservation["RaisonSociale"])): ?>
                                        <br><small style="color: var(--text-secondary);">(<?= htmlspecialchars($reservation["RaisonSociale"]) ?>)</small>
                                    <?php endif; ?>
                                </td>
                                <td data-label="Date début"><?= htmlspecialchars($reservation["date_debut"]) ?></td>
                                <td data-label="Date fin"><?= htmlspecialchars($reservation["date_fin"]) ?></td>
